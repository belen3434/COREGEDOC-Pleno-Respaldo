...
try {
    ...
    if (!$auth['authorized'] || !in_array((int)($auth['tipoUsuarioId'] ?? 0), [6, 20], true)) {
        ...
        echo json_encode([
            'success' => false,
            'mensaje' => 'No tiene permisos para iniciar el pleno.',
    ...
    if (trim((string)($sesion['estado'] ?? '')) !== 'programada') {
        http_response_code(409);
        echo json_encode([
            'success' => false,
            'mensaje' => 'Solo se puede iniciar una sesión programada.',
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    if (!$actualizado) {
        ...
        echo json_encode([
            'success' => false,
            'mensaje' => 'No fue posible iniciar el pleno.',
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    echo json_encode([
        'success' => true,
        'estado' => 'en_curso',
        'mensaje' => 'Pleno iniciado correctamente.',
    ], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
    http_response_code(500);
    echo json_encode([
