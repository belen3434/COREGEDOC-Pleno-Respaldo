...
$resumenes = $data['pleno_historial_resumenes'] ?? [];
$filtros = $data['pleno_historial_resumenes_filtros'] ?? [
    'numero_sesion' => '',
    'fecha' => '',
    'estado' => 'todos',
    'numero_resumen' => '',
...
<div class="container-fluid mt-4 historial-resumenes-page">
    <div class="historial-heading">
        <span class="historial-heading-icon"><i class="fas fa-file-alt"></i></span>
        <div>
            <h1 class="historial-title">Historial de Res&uacute;menes Ejecutivos</h1>
            <p class="historial-subtitle">Consultar res&uacute;menes ejecutivos generados por sesi&oacute;n.</p>
        </div>
        <a href="index.php?vista=historiales" class="btn btn-pleno-back ms-auto">
            <i class="fas fa-arrow-left me-2"></i>Atr&aacute;s
        </a>
    </div>
    ...
    <section class="historial-card">
        <div class="historial-card-body">
            <form class="historial-filters" method="get" action="index.php">
                <input type="hidden" name="vista" value="historial_resumenes">
                <div>
                ...
                <div>
                    <label class="historial-label" for="historialNumeroResumen">N&uacute;mero resumen</label>
                    <input
                        class="historial-input"
                        id="historialNumeroResumen"
                        name="numero_resumen"
                        type="text"
                        value="<?php echo $h($filtros['numero_resumen'] ?? ''); ?>"
                        placeholder="RES-EJEC-PL-2026-034-V1"
                    >
                </div>
    ...
    <section class="historial-card">
        <div class="historial-card-body">
            <?php if (empty($resumenes)): ?>
                <p class="mb-0 text-muted">No se encontraron res&uacute;menes ejecutivos con los filtros seleccionados.</p>
            <?php else: ?>
                <div class="historial-table-wrap">
                    <table class="historial-table">
                        <thead>
                            <tr>
                                <th>N&uacute;mero resumen</th>
                                <th>Sesi&oacute;n</th>
                                <th>Tipo de pleno</th>
                        ...
                        <tbody>
                            <?php foreach ($resumenes as $resumen): ?>
                                <?php
                                $idResumen = (int)($resumen['id_resumen'] ?? 0);
                                $estado = (string)($resumen['estado'] ?? 'Sin estado');
                                ?>
                                <tr>
                                    <td><strong><?php echo $h($resumen['numero_resumen'] ?? 'Sin n&uacute;mero'); ?></strong></td>
                                    <td><?php echo $h($resumen['numero_sesion'] ?? 'Sin sesi&oacute;n'); ?></td>
                                    <td><?php echo $h($resumen['tipo_pleno'] ?? 'Sin tipo'); ?></td>
                                    <td><?php echo $h($formatearFecha($resumen['fecha_sesion'] ?? null)); ?></td>
                                    <td>V<?php echo (int)($resumen['version'] ?? 1); ?></td>
                                    <td><?php echo $h($formatearFechaHora($resumen['fecha_generacion'] ?? null)); ?></td>
                                    <td><?php echo (int)($resumen['total_asistentes'] ?? 0); ?></td>
                                    <td><?php echo (int)($resumen['total_votaciones'] ?? 0); ?></td>
                                    <td><?php echo (int)($resumen['total_acuerdos'] ?? 0); ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="historial-actions">
                                            <a class="btn btn-outline-primary btn-sm" target="_blank" rel="noopener" href="resumenes/ver.php?id_resumen=<?php echo $idResumen; ?>">
                                                <i class="fas fa-eye me-1"></i>Ver
                                            </a>
                                            <a class="btn btn-outline-success btn-sm" href="resumenes/descargar.php?id_resumen=<?php echo $idResumen; ?>">
